...
class TenderProcuringEntityName:
	def load_count_map(self, row: tuple) -> int:
		count = 0
		if row[self.colNumber]:
			if('procuringEntity' in row[self.colNumber]):
				procuringEntity = row[self.colNumber]['procuringEntity']
				value = procuringEntity['name']
	...
	def process_row(self, row: tuple, colNumber: int):
		idArr = [0, 0, 0, 0]
		if row[colNumber]:
			if('procuringEntity' in row[colNumber]):
				procuringEntity = row[colNumber]['procuringEntity']
				value = procuringEntity['name']
