...
class TendersubmissionMethodDetails:
	def load_count_map(self, row: tuple) -> int:
		count = 0
		if row[self.colNumber]:
			if('submissionMethodDetails' in row[self.colNumber]):
				value = row[self.colNumber]['submissionMethodDetails']
	...
	def process_row(self, row: tuple, colNumber: int):
		idArr = [0, 0, 0, 0]
		if row[colNumber]:
			if('submissionMethodDetails' in row[colNumber]):
				value = row[colNumber]['submissionMethodDetails']
