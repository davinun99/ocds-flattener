...
class TenderEligibilityCriteria:
	def load_count_map(self, row: tuple) -> int:
		count = 0
		if row[self.colNumber]:
			if('eligibilityCriteria' in row[self.colNumber]):
				value = row[self.colNumber]['eligibilityCriteria']
	...
	def process_row(self, row: tuple, colNumber: int):
		idArr = [0, 0, 0, 0]
		if row[colNumber]:
			if('eligibilityCriteria' in row[colNumber]):
				value = row[colNumber]['eligibilityCriteria']
