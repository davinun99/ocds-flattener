...
class TenderMainProcurementCategoryDetails:
	def load_count_map(self, row: tuple) -> int:
		count = 0
		if row[self.colNumber]:
			if('mainProcurementCategoryDetails' in row[self.colNumber]):
				value = row[self.colNumber]['mainProcurementCategoryDetails']
	...
	def process_row(self, row: tuple, colNumber: int):
		idArr = [0, 0, 0, 0]
		if row[colNumber]:
			if('mainProcurementCategoryDetails' in row[colNumber]):
				value = row[colNumber]['mainProcurementCategoryDetails']
